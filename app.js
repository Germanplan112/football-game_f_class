...
function checkZone(x, y, isStart){
    ...
    if (x < width / 2) {
        ...
        if (y > height / 2 + 60) {
            keys['KeyA'] = isStart && (x < 3 * width / 4);
            keys['KeyD'] = isStart && (x >= 3 * width / 4);
        }
    }
}

// Объекты
const p1 = {x: 100, y: 0, w: 50, h: 50, speed: 8};
const p2 = {x: 100, y: 0, w: 50, h: 50, speed: 8};
const ball = {x: 0, y: 0, r: 15, vx: 0, vy: 0, heldBy: null};
const goalL = {x: -10, y: height/2 - 50, w: 10, h: 100};
const goalR = {width - 0, y: height/2 - 50, w: 10, h: 100};
...
function resizeCanvas(){
    ...
    height = window.innerHeight - 60; // отступ под счет
    canvas.width = width;
...
function update(){
    ...
    if(gameMode === 'pvp'){
        if(keys['KeyA']) p2.x -= p2.speed;
        if(keys['KeyD']) p2.x += p2.speed;
    } else if(gameMode === 'bot'){ // Простейший бот
    ...
    if(ball.heldBy){
        ...
        if(keys['Space'] || Math.abs(ball.vy) > 0.1) {} // заглушка для клавиатуры
    } else {
        ball.x += ball.vx;
        ball.y += ball.vy;
        ball.vy += 0.3; // гравитация
